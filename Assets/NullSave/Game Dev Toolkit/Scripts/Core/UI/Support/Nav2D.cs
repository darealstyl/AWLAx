...
{
    [Serializable]
    [AutoDocLocation("ui/classes")]
    [AutoDoc("Provides 2d navigation input")]
    public class Nav2D
    {
        ...
        [Tooltip("Event raised on next (horizontal)")] public SimpleEvent onNextH;
        [Tooltip("Event raised on previous (horizontal)")] public SimpleEvent onBackH;
        [Tooltip("Event raised on next (vertical)")] public SimpleEvent onNextV;
        [Tooltip("Event raised on previous (vertical)")] public SimpleEvent onBackV;
        ...
        {
            if (lockInput || InterfaceManager.IsBlockedByModal(gameObject)) return;
