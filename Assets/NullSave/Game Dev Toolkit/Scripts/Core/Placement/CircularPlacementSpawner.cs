...
namespace NullSave.GDTK
{
    ...
    [AutoDoc("The Circular Placement Spawner component spawns items using a Circular Placement component.")]
    ...
    public class CircularPlacementSpawner : MonoBehaviour, IBroadcastReceiver
    {
        ...
        [Tooltip("List of weighted entries")] public List<WeightedUnityEntry> entries;

        [Tooltip("Respond to messages from the Broadcaster")] public bool respondToBroadcasts;
        ...
        [AutoDoc("Generate spawn based on weighted lists")]
        public void SpawnItems()
        {
            ...
            foreach (WeightedUnityItem item in items)
            {
                ...
                {
                    StringExtensions.Log(gameObject, "CircularPlacment", "Non-GameObject supplied, skipping");
                }
            }
            ...
            foreach (WeightedUnityItem item in finalItems)
            {
                for (int i = 0; i < item.Count; i++)
                {
                    ...
                    {
                        StringExtensions.Log(gameObject, "CircularPlacment", "Ran out of available spots early, ending prematurely");
                        break;
                    }
