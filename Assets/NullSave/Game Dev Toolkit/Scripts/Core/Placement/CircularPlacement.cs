...
{
    [AutoDocLocation("circular-placement-system")]
    [AutoDoc("The Circular Placement component creates a set of rings around an object for object placement.")]
    public class CircularPlacement : MonoBehaviour
    {
        ...
        [AutoDoc("List of rings for placement")] public List<CircularPlacementRing> rings;

        #endregion

        #region Properties

        [AutoDoc("List of all locations")]
        public List<Vector3> AllRelativeLocations { get; private set; }

        [AutoDoc("List of locations claimed or reserved")]
        ...
        private void OnDrawGizmosSelected()
        {
            ...
            foreach (CircularPlacementRing ring in rings)
            {
                ...
                while (degrees < 360)
                {
                    ...
                    if (procCount > 150)
                    {
                        StringExtensions.Log(gameObject, "CircularPlacment", "Too many gizmos, exiting for safey");
                        return;
                    }
                }
            }
        ...
        [AutoDoc("Gets first available position, returns Vector3.zero if not spots available")]
        [AutoDocParameter("Exclude locations listed in ClaimedRelativeLocations")]
        ...
        [AutoDoc("Finds a random free position, returns Vector3.zero if not spots available")]
        [AutoDocParameter("Exclude locations listed in ClaimedRelativeLocations")]
        ...
        [AutoDoc("Finds a list of random free positions")]
        [AutoDocParameter("Number for free positions to find")]
        [AutoDocParameter("Exclude locations listed in ClaimedRelativeLocations")]
