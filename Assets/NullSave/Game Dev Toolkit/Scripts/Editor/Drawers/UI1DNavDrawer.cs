...
namespace NullSave.GDTK.Inventory
{
    ...
    public class UI1DNavDrawer : GDTKPropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (flags.HasFlag(InputFlags.Key))
            {
                SimpleEnumContext(rect, property.FindPropertyRelative("backKey"), "Back Code", typeof(KeyCode));
                rect.y += rect.height;
                SimpleEnumContext(rect, property.FindPropertyRelative("nextKey"), "Next Code", typeof(KeyCode));
                rect.y += rect.height;
            }
            ...
            rect.y += rect.height;
            EditorGUI.PropertyField(rect, property.FindPropertyRelative("ignoreModal"));
            rect.y += rect.height;
