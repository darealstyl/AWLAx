...
namespace NullSave.GDTK.Inventory
{
    ...
    public class Nav2DDrawer : GDTKPropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (flags.HasFlag(InputFlags.Key))
            {
                SimpleEnumContext(rect, property.FindPropertyRelative("leftKey"), "Left Code", typeof(KeyCode));
                rect.y += rect.height;
                SimpleEnumContext(rect, property.FindPropertyRelative("rightKey"), "Right Code", typeof(KeyCode));
                rect.y += rect.height;
                SimpleEnumContext(rect, property.FindPropertyRelative("upKey"), "Up Code", typeof(KeyCode));
                rect.y += rect.height;
                SimpleEnumContext(rect, property.FindPropertyRelative("downKey"), "Down Code", typeof(KeyCode));
                rect.y += rect.height;
            }
